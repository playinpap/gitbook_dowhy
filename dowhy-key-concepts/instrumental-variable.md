...
$$E(x\_2y)$$를 $$n^{-1}\sum\_{i=1}^{n}x\_{i2}y\_i$$ 추정하는 등의 방식을 사용하여 모집단 상수를 구한 후 위의 등식에 대입한다면 $$\beta_0$$, $$\beta_1$$, $$\beta_2$$가 결정된다. (단, 비특이성을 만족시킨다: 3원 1차 연립방정식의 해가 유일할 조건이 있다.)

하지만, $$x_2$$가 내생적이라면 $E(x\_2, u) \neq 0$ 이고
